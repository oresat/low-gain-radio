...
bool rb_put_elem(RB_ELEM c, Ringbuffer * rb)
{
	...
	if((rb->num_entries) < (rb->max_entries))
	{
		...
		if((rb->num_entries) < (rb->max_entries))
		{
			rb->tailidx = (rb->tailidx + 1) % rb->max_entries;
		}
	}
	else
	{
...
bool rb_get_elem(RB_ELEM * c, Ringbuffer * rb)
{
	...
	if(rb->num_entries != 0)
	{
		*c = rb->rbuff[rb->headidx];
		...
		rb->num_entries--;
		if((rb->headidx) < (rb->max_entries))
		{
			rb->headidx = (rb->headidx + 1) % rb->max_entries;
		}
	}
	else
	{
