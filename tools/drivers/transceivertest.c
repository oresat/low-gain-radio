...
void trans_trans_read_register(uint8_t address, uint8_t * buffer, uint8_t length){
	spi0_transaction_8(length, &address, 0, buffer);
}

void trans_trans_write_register(uint8_t address, uint8_t * buffer, uint8_t length){
	address |= 0x80; //signal transceiver that this is a write op
	spi_write(length, &address, buffer, 0);
}

void trans_rw_register(uint8_t address, uint8_t * send, uint8_t * recv, uint8_t length){
	address |= 0x80; //signal transceiver that this is a write op
	spi_write(length, &address, send, recv);
}
